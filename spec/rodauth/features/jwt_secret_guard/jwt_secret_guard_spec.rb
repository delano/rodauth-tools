...
RSpec.describe "JwtSecretGuard" do
  ...
  describe "basic functionality" do
    ...
    it "does not interfere when jwt_secret is already set" do
      app = create_roda_app do
        enable :jwt_secret_guard
      ...
      expect(rodauth_instance.jwt_secret).to eq('my-custom-secret-12345')
    end
  end
  ...
  describe "production mode validation" do
    ...
    it "succeeds in production when secret is set" do
      app = create_roda_app do
        enable :jwt_secret_guard
        production_env_check true
        jwt_secret 'production-secret-12345'
      end

      expect(app).not_to be_nil
      rodauth_instance = app.rodauth.allocate
      expect(rodauth_instance.jwt_secret).to eq('production-secret-12345')
    end
    ...
    it "raises error with custom message" do
      ...
      end.to raise_error(Rodauth::ConfigurationError, 'CRITICAL: JWT secret not configured!')
    end
  end
    ...
    it "uses custom development fallback" do
      output = capture_warnings do
        app = create_roda_app do
          enable :jwt_secret_guard
          production_env_check false
          development_jwt_secret_fallback 'custom-dev-secret'
        end

        expect(app).not_to be_nil
        rodauth_instance = app.rodauth.allocate
        expect(rodauth_instance.jwt_secret).to eq('custom-dev-secret')
      end
    ...
    it "uses custom warning message" do
      output = capture_warnings do
        create_roda_app do
          enable :jwt_secret_guard
          production_env_check false
          jwt_secret_dev_warning 'DEV WARNING: Custom message'
        end
      end
  ...
  describe "environment variable loading" do
    before do
      ENV['JWT_SECRET'] = 'env-secret-12345'
    end
    ...
    it "loads secret from environment variable" do
      ...
      expect(app).not_to be_nil
      rodauth_instance = app.rodauth.allocate
      expect(rodauth_instance.jwt_secret).to eq('env-secret-12345')
    end
    ...
    it "deletes secret from ENV after loading" do
      ...
      expect(ENV['JWT_SECRET']).to be_nil
    end
    ...
    it "uses custom environment variable key" do
      ...
      app = create_roda_app do
        enable :jwt_secret_guard
        jwt_secret_env_key 'MY_CUSTOM_SECRET'
        production_env_check true
      end

      expect(app).not_to be_nil
      rodauth_instance = app.rodauth.allocate
      expect(rodauth_instance.jwt_secret).to eq('custom-env-secret')
      expect(ENV['MY_CUSTOM_SECRET']).to be_nil
    ...
    it "prefers explicitly set secret over environment variable" do
      ...
      rodauth_instance = app.rodauth.allocate
      expect(rodauth_instance.jwt_secret).to eq('explicit-secret')
      expect(ENV['JWT_SECRET']).to eq('env-secret-12345')
    end
  end
